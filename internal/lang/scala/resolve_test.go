...
func TestClassifyStd(t *testing.T) {
	...
	for _, pkg := range []string{
		"scala", "scala.collection.mutable", "scala.concurrent",
		"java.util", "javax.swing", "Predef",
	} {
...
func TestClassifyExternal(t *testing.T) {
	declared := map[string]string{}
	for _, pkg := range []string{
		"cats.effect", "org.scalatest", "io.circe",
		"scalafoo.bar", // NOT the scala stdlib: segment boundary matters
...
func TestClassifyInModule(t *testing.T) {
	declared := map[string]string{
		"com.example.domain":  "src/main/scala/com/example/domain",
		"com.example.service": "src/main/scala/com/example/service",
	}
	...
	if relDir != "src/main/scala/com/example/domain" {
		t.Errorf("relDir = %q, want src/main/scala/com/example/domain", relDir)
	}
...
func TestClassifyInModuleWinsOverStdShape(t *testing.T) {
	// A first-party package that happens to sit under a std-looking prefix is still
	// in-module because it is declared by the project.
	declared := map[string]string{"scala.internal.tool": "src/main/scala/scala/internal/tool"}
	ref := importRef{Pkg: "scala.internal.tool", Display: "scala.internal.tool.Helper"}
	class, relDir, _, ok := classify(ref, declared)
	if !ok || class != core.ClassInModule || relDir != "src/main/scala/scala/internal/tool" {
...
func TestClassifyUndeclaredIsExternal(t *testing.T) {
	...
	declared := map[string]string{"com.example.domain": "src/main/scala/com/example/domain"}
...
func TestIsStdlib(t *testing.T) {
	std := []string{"scala", "scala.collection", "scala.concurrent", "java.util", "javax.swing", "Predef"}
	for _, p := range std {
		if !isStdlib(p) {
			t.Errorf("isStdlib(%q) = false, want true", p)
		}
	}
	notStd := []string{"com.example", "scalafoo", "javafoo", "org.scalalib", "predef", ""}
