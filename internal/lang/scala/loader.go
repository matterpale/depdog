// Package scala is the Scala language adapter: it statically scans a project's
// .scala source files for their leading `package` declaration and `import`
// statements (import a.b.C, import a.b.{C, D}, import a.b.{C => E}, import a.b._,
// import a.b.*, and import a.b.given), resolves each import against the set of
// packages the project itself declares, and builds the same directory-keyed
// *core.Graph the Go, TypeScript, Python, Rust, Java and Kotlin adapters produce.
// It runs no JVM/Scala compiler and needs no build tool (sbt/Mill) — it reads
...
// Loader loads the Scala project rooted at (or above) Dir.
type Loader struct {
	Dir string
}
...
// build.sbt is sbt; build.sc is Mill.
var markers = []string{"build.sbt", "build.sc"}
...
func (l *Loader) Load(_ context.Context, patterns ...string) (*core.Graph, error) {
	...
	// imports, and build the declared-package -> node-dir map that classification
	// consults. A node dir is a file's source-root-relative directory (e.g.
	...
	for _, abs := range files {
		...
		if res.pkg != "" {
			// A package can be split across source roots (src/main + src/test). The
			// first node dir seen (files are sorted) wins deterministically;
			// production source sorts before test source under the standard layout,
			// so edges resolve to the main directory.
...
// projectRoot walks up from start looking for one of the Scala project markers,
...
func projectRoot(start string) (string, error) {
	...
	return "", fmt.Errorf("no %s found from %s upward; "+
		"run depdog from inside a Scala project (the directory that holds %s)",
		strings.Join(markers, ", "), abs, strings.Join(markers, ", "))
}

// discoverFiles walks root and collects .scala files, skipping build output and
...
// (including .bsp/.metals) and the conventional sbt/Mill build-output
...
func skipDir(name string) bool {
	...
	switch name {
	case "target", "out", "project", "node_modules":
		// sbt writes to target/; Mill writes to out/; the sbt `project/` directory
		// holds meta-build sources (plugins.sbt, *.scala) that are build config, not
		// application source.
		return true
	}
	return false
...
// the root node using ModulePath alone — matching the Go/TS/Py/Rust/Java/Kotlin
// golden shape.
...
// isTestFile marks conventional Scala test sources: any file under a src/test/
// tree (the standard sbt test source root).
