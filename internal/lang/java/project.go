...
// of every node's display ImportPath. It reads a project name from pom.xml
// (<artifactId>), build.gradle / build.gradle.kts (rootProject.name = "…"),
// falling back to the root directory's basename. The reads are tiny hand-rolled
// scans — enough to find one value without pulling in an XML/Groovy parser,
// keeping the adapter std-lib only.
func projectName(root string) string {
	if name := artifactIDFromPom(filepath.Join(root, "pom.xml")); name != "" {
		return name
	}
	for _, marker := range []string{"build.gradle.kts", "build.gradle", "settings.gradle.kts", "settings.gradle"} {
...
// artifactIDFromPom returns the top-level <artifactId> of a Maven pom.xml. It
// deliberately picks the first <artifactId> that is NOT nested inside a
// <parent>, <dependency>, <plugin>, or similar block so the project's own id is
// chosen over a parent/dependency coordinate. Returns "" if the file is absent
// or has no usable id.
