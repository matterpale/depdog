...
// Find walks up from startDir to the nearest go.mod and expects the config
// beside it. It refuses to run inside a Go workspace: v1 checks exactly one
// module.
func Find(startDir string) (configPath, moduleRoot string, err error) {
	...
	for d := dir; ; {
		...
		if w := filepath.Join(d, "go.work"); exists(w) && os.Getenv("GOWORK") != "off" {
			return "", "", fmt.Errorf("go workspaces are not supported yet: found %s — depdog v1 checks a single module (run with GOWORK=off to bypass the workspace)", w)
		}
	...
	if gw := os.Getenv("GOWORK"); gw != "" && gw != "off" {
		return "", "", fmt.Errorf("go workspaces are not supported yet: GOWORK=%s — depdog v1 checks a single module", gw)
	}
	if root == "" {
